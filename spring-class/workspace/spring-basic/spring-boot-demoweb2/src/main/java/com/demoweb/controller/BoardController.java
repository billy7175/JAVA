...
public class BoardController {
	...
	public String list(
		...
		List<BoardEntity> boards = boardService.findAllBoardByPage(from, pageSize); // 페이지에 해당하는 데이터만 조회
		long boardCount = boardService.findBoardCount();	// 전체 데이터 갯수
	...
	public String write(BoardEntity board, MultipartHttpServletRequest req) {		
		...
		MultipartFile attach = req.getFile("attach"); // <input type="file" name="attach" 에 입력된 파일 읽기
		...
		if (attach != null && !attach.isEmpty()) {
			...
			String path = // 웹경로 (http://ip:port/path1/path2/...) -> 컴퓨터경로 (C:\path1\path2\...)
	...
	public View download(
		...
		if (attach == null) {
			...
			model.addAttribute("attach", attach); // View에서 읽을 수 있도록 데이터 저장
