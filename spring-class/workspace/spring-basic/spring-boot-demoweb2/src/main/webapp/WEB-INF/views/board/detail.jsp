...
" />
	...
	$(function() {
		...
		$('#write-comment-btn').on('click', function(event) {
			...
			var data = $('#comment-form').serialize(); // <form>...</form>의 데이터를 전송가능한 문자열로 변경 "boardno=xxx&writer=yyy&content=zzz"
			...
			$.ajax({
				...
				"success": function(data, status, xhr) {
					...
					//목록 갱신 (ajax)
					// load : ajax 방식으로 요청 + 응답은 html + 지정된 요소의 화면내용으로 갱신
		...
		//$('.deletecomment').on('click', function(event) { // 현재 페이지에 있는 객체에 대해서만 이벤트 구성
		$('#comment-list').on('click', '.deletecomment', function(event) { // 현재 페이지에 없는 객체에 대해서도 이벤트 구성 
			...
			//ajax 방식으로 데이터 삭제
			$.ajax({
				"url": "delete-comment/" + commentNo, // delete-comment/10    // 비교 : "/delete-comment?commentNo=" + commentNo
				"method": "GET",
				"success": function(data, status, xhr) {					
					alert('댓글을 삭제 했습니다.');
					$('#comment-list tbody').load("get-comment-list/" + ${ board.boardNo });					
